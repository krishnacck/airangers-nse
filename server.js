...
app.listen(PORT, () => {
  ...
  // Auto-connect: activate simulated mode directly (no HTTP needed)
  fyersAuth.autoConnect();
  ...
  if (process.env.AUTO_START_SCHEDULER === "true") {
    ...
    console.log(`⚡ Auto-trader started (scan every ${process.env.SCAN_INTERVAL_SECONDS || 15}s)`);
  }
  ...
  if (process.env.AUTO_START_DATADUMP !== "false") {
    ...
    if (client && !fyersAuth.isSimulated()) {
      ...
      console.log("📡 Data dumper started (live)");
    } else {
